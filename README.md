...
Extracts current weather for 10 cities from the OpenWeather API
Transforms the nested JSON responses into a flat, typed table
Loads the result into a CSV file and a SQLite database
Analyses the data to compare temperature, humidity and conditions across cities
...
Repository Contents
File	Description
etl_pipeline.py	Standalone pipeline script — run this to execute the full ETL
Weather_ETL_Pipeline.ipynb	Development notebook with exploration, analysis and visualisation
...
CSV — weather_data.csv, appended with headers written only on first creation
SQLite — weather.db, table weather, using if_exists="append"
...
The API key is never stored in this repository. It is read from an environment variable at runtime, and during development was held in Google Colab's secrets manager. Anyone running this project supplies their own key, free from openweathermap.org.

Steps Taken
Created an OpenWeather account and generated an API key
Stored the key as a Colab secret, outside the notebook file
Tested a single API call and inspected the JSON structure
Wrote an extraction function with timeout and error handling
Collected all 10 cities in a loop, with pacing between requests
Flattened the nested responses into a pandas DataFrame
Renamed columns, verified data types, checked for missing values
Added derived columns for perceived-temperature difference and collection time
Saved to CSV and SQLite in append mode
Analysed temperature, humidity and condition spread across cities
Refactored the notebook into a standalone etl_pipeline.py script
Key Findings
...
The most humid city was also the coldest. Reykjavik recorded 76% humidity against Pretoria's 27%. This is not a contradiction — relative humidity measures how close the air is to saturation, and cold air saturates at much lower moisture content. A cold city at 76% and a tropical city at a similar reading feel nothing alike, because they hold very different amounts of actual moisture. Absolute humidity and relative humidity are different measurements, and only the second one is in this dataset.
